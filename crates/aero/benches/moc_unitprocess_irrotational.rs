...
fn test_interior_point_2_variable() {
    let config = UnitprocessConfig {
        axisym: AreaType::Axisymmetric,
        tol: Tolerance::new(1e-5, 1e-5),
        n_corr: 20,
...
fn bench_interior_point(c: &mut Criterion) {
    c.bench_function(
        "moc::unitprocess::irrotational::interior_point() constant cp",
        |b| {
            b.iter(|| {
    ...
    c.bench_function(
        "moc::unitprocess::irrotational::interior_point() variable cp",
        |b| {
            b.iter(|| {
                // test_interior_point_1_variable();
                test_interior_point_2_variable();
            })
        },
    );
}
